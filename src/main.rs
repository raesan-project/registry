...
fn main() {
    ...
    let mut subjects_vec = serde_json::from_str::<Vec<models::Subject>>(&contents).unwrap();
    ...
    for chapter_file in fs::read_dir(chapters_dir).unwrap() {
        ...
        for chapter in curr_chapters_vec.iter_mut() {
            let curr_subject = subjects_vec
                .iter()
                .find(|subject| subject.name == chapter.subject_name)
                .unwrap();
            chapter.subject_id = curr_subject.id.clone();
            chapter.id = uuid::Uuid::new_v4().to_string();
            chapter.updated_at = 1191105900;
            chapter.created_at = 1191105900;
        }
        fs::write(
            ...
            if contents.trim().len() != 0 {
                ...
                for question in questions_vec.iter_mut() {
                    let curr_chapter = glob_chapters_vec
                        .iter()
                        .find(|chapter| chapter.name == question.chapter_name)
                        .unwrap();
